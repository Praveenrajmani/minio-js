...
import {
    isValidPrefix, isValidEndpoint, isValidBucketName,
    ...
    probeContentType, makeDateLong, promisify
} from './helpers.js'
...
export class Client {
    ...
    makeRequest(options, payload, statusCode, region, returnResponse, cb) {
        ...
        if (this.enableSHA256) {
            sha256sum = Crypto.createHash('sha256').update(payload).digest('hex')
        }
    ...
    // Remove all the objects residing in a bucket.
    //
    // __Arguments__
    // * `bucketName` _string_: name of the bucket
    // * `objectsCh` _string_: channel of objects
    removeObjects(bucketName, objectsCh) {
        if (!isValidBucketName(bucketName)) {
            throw new errors.InvalidBucketNameError('Invalid bucket name: ' + bucketName)
        }
        if (typeof objectsCh !== "object") {
            throw new errors.InvalidArgumentError('objectsCh should be a channel')
        }
        ...
        readStream._read = async () => {
            ...
            while (true) {
                ...
                var chObject = await objectsCh.shift()

                while (chObject !== undefined) {
                    batch.push(chObject);
                    if (++count >= maxEntries) {
                        break;
                    }
                    chObject = await objectsCh.shift()
                }
                ...
                var payload = this.generateRemoveMultiObjectsRequest(batch)
                var headers = {}
                var md5digest = Crypto.createHash('md5').update(payload).digest()

                headers['Content-MD5'] = md5digest.toString('base64')

                this.makeRequest({ method: "POST", bucketName: bucketName, query: query, headers: headers }, payload, 200, '', true, (e, response) => {
                    if (e) {
    ...
    generateRemoveMultiObjectsRequest(objects) {
        ...
        var xmlBytes = Xml(deleteObjects)
        return xmlBytes
    }
